...
# Google Drive 파일 ID
file_id = '1ZFoJCV-qumYeDYjsHghh5vM8ioHsHubM'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/z74NRzV/Cz6m-Zvz-Zn-R2vy-HUWic41tq-Cn-DKkb-UQp-Wli-EXMqw5-CH9-Eq-Kr-Tzv-Uqj-Foo-JTEn0-DWPdtw-F7-Td-H8i5-F6t.webp?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=4dTzktjYIx4", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 앙기모찌 이재환")
